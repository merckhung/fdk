...
void updateStatusTimer(fdkUiProperty_t *pFdkUiProperty) {
  ...
  if (pFdkUiProperty->fdkBasePanel.lastSecond < thisSecond) {
    ...
    printWindowAt(pFdkUiProperty->fdkBasePanel, time, FDK_STRING_NLINE,
        FDK_MAX_TIMESTR, FDK_MAX_LINE, FDK_MAX_COLUMN - FDK_MAX_TIMESTR,
        BLACK_WHITE, "%2.2d:%2.2d:%2.2d", nowtime->tm_hour, nowtime->tm_min,
        nowtime->tm_sec);

    update = 1;
  }

  if (pFdkUiProperty->fdkBasePanel.statusStr
      && ((thisSecond - pFdkUiProperty->fdkBasePanel.lastSecond)
          >= FDK_STS_INTV_SECS)) {
    ...
    if ((len - pFdkUiProperty->fdkBasePanel.strIdx)
        > (FDK_MAX_COLUMN - FDK_MAX_TIMESTR))
      len = FDK_MAX_COLUMN - FDK_MAX_TIMESTR;

    snprintf(stsBuf, len, "%s",
        (pFdkUiProperty->fdkBasePanel.statusStr
            + pFdkUiProperty->fdkBasePanel.strIdx));

    // Update status bar
    printWindow(pFdkUiProperty->fdkBasePanel, status, FDK_STRING_NLINE,
        FDK_MAX_COLUMN - FDK_MAX_TIMESTR, FDK_MAX_LINE, FDK_MIN_COLUMN,
        BLACK_WHITE, "%s", stsBuf);
...
void printBasePlane(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Background Color
  printWindow(pFdkUiProperty->fdkBasePanel, background, FDK_MAX_LINE,
      FDK_MAX_COLUMN, FDK_MIN_LINE, FDK_MIN_COLUMN, WHITE_BLUE, "");

  // Logo
  printWindow(pFdkUiProperty->fdkBasePanel, logo, FDK_STRING_NLINE,
      FDK_MAX_COLUMN, FDK_MIN_LINE, FDK_MIN_COLUMN, WHITE_RED,
      CFDK_PROGRAM_NAME);

  // Copyright
  printWindow(pFdkUiProperty->fdkBasePanel, copyright, FDK_STRING_NLINE,
      strlen( FDK_AUTHOR_NAME ), FDK_MIN_LINE,
      FDK_MAX_COLUMN - strlen( FDK_AUTHOR_NAME ), WHITE_RED, FDK_AUTHOR_NAME);
...
";
  ...
  if (pFdkUiProperty->fdkBasePanel.toggleHelp) {
    ...
    printWindowMove(pFdkUiProperty->fdkBasePanel, help, (FDK_MAX_LINE - 4),
        (FDK_MAX_COLUMN - 4), FDK_HELP_X_POS, FDK_HELP_Y_POS, BLACK_WHITE, "%s",
        helpTxt);
  } else
...
s32 main(s32 argc, s8 **argv) {
  ...
  for (;;) {
    ...
    switch (fdkUiProperty.inputBuf) {
      ...
      case KBPRS_ESC:
        ...
        // Help
      case KBPRS_F1:
        fdkUiProperty.fdkBasePanel.toggleHelp =
            !fdkUiProperty.fdkBasePanel.toggleHelp;
        break;
    ...
    if (fdkUiProperty.fdkPreviousHwFunc != fdkUiProperty.fdkHwFunc) {
  ...
  ErrExit1: free(fdkUiProperty.pFdkPciDev);
