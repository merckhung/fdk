...
void printDumpBasePanel(fdkUiProperty_t *pFdkUiProperty) {
  ...
  printWindowAt(pFdkUiProperty->fdkDumpPanel, top, FDK_STRING_NLINE,
      strlen( FDK_DUMP_TOP_BAR ), FDK_DUMP_TOP_LINE, FDK_DUMP_TOP_COLUMN,
      GREEN_BLUE, FDK_DUMP_TOP_BAR);

  // Print Rtop bar
  if (pFdkUiProperty->fdkHwFunc != KHF_PCI)
    printWindowAt(pFdkUiProperty->fdkDumpPanel, rtop, FDK_STRING_NLINE,
        FDK_DUMP_BYTE_PER_LINE, FDK_DUMP_RTOP_LINE, FDK_DUMP_RTOP_COLUMN,
        RED_BLUE, FDK_DUMP_RTOP_BAR);

  // Print Left bar
  printWindowAt(pFdkUiProperty->fdkDumpPanel, left, FDK_DUMP_BYTE_PER_LINE, 4,
      FDK_DUMP_LEFT_LINE, FDK_DUMP_LEFT_COLUMN, GREEN_BLUE, FDK_DUMP_LEFT_BAR);

  // Print Info base
  printWindowAt(pFdkUiProperty->fdkDumpPanel, info, FDK_STRING_NLINE,
      FDK_MAX_COLUMN, FDK_INFO_LINE, FDK_INFO_COLUMN, WHITE_BLUE, "%s",
...
void printDumpUpdatePanel(fdkUiProperty_t *pFdkUiProperty) {
  ...
  switch (pFdkUiProperty->fdkHwFunc) {
    ...
    case KHF_PCI:
      dataPtr =
          (u8 *) &pFdkUiProperty->pFdkCommPkt->fdkRspPciReadPkt.pciContent;
      break;
    ...
    case KHF_IDE:
      dataPtr =
          (u8 *) &pFdkUiProperty->pFdkCommPkt->fdkRspIdeReadPkt.ideContent;
      break;

    case KHF_CMOS:
      dataPtr =
          (u8 *) &pFdkUiProperty->pFdkCommPkt->fdkRspCmosReadPkt.cmosContent;
      break;

    default:
    case KHF_MEM:
      dataPtr =
          (u8 *) &pFdkUiProperty->pFdkCommPkt->fdkRspMemReadPkt.memContent;
      break;
  }
  pDataPtr = dataPtr;
  ...
  for (i = 0; i < FDK_DUMP_BYTE_PER_LINE; i++) {
    ...
    vp +=
        sprintf((s8 *) vp,
            ...
            dataPtr[5], dataPtr[6], dataPtr[7], dataPtr[8], dataPtr[9],
            dataPtr[10], dataPtr[11], dataPtr[12], dataPtr[13], dataPtr[14],
            dataPtr[15]);

    if (pFdkUiProperty->fdkHwFunc != KHF_PCI)
      ap += sprintf((s8 *) ap, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c",
          FDK_DUMP_ASCII_FILTER(dataPtr[0]), FDK_DUMP_ASCII_FILTER(dataPtr[1]),
          FDK_DUMP_ASCII_FILTER(dataPtr[2]), FDK_DUMP_ASCII_FILTER(dataPtr[3]),
          FDK_DUMP_ASCII_FILTER(dataPtr[4]), FDK_DUMP_ASCII_FILTER(dataPtr[5]),
          FDK_DUMP_ASCII_FILTER(dataPtr[6]), FDK_DUMP_ASCII_FILTER(dataPtr[7]),
          FDK_DUMP_ASCII_FILTER(dataPtr[8]), FDK_DUMP_ASCII_FILTER(dataPtr[9]),
  ...
  if (pFdkUiProperty->fdkHwFunc == KHF_PCI) {
    ...
    sprintf((s8 *) ap, "VEN ID: %4.4Xh\n"
        ...
        "%s: %8.8Xh\n"
        "%s: %8.8Xh\n\n"
        "ROM: %8.8Xh\n", pFdkPciConfig->vendorId, pFdkPciConfig->deviceId,
        pFdkPciConfig->revisionId, pFdkPciConfig->intLine,
  ...
  // Print value
  printWindowAt(pFdkUiProperty->fdkDumpPanel, value, FDK_DUMP_BYTE_PER_LINE,
      FDK_DUMP_BUF_PER_LINE, FDK_DUMP_VALUE_LINE, FDK_DUMP_VALUE_COLUMN,
      WHITE_BLUE, "%s", valueBuf);

  // Print ASCII
  printWindowAt(pFdkUiProperty->fdkDumpPanel, ascii, FDK_DUMP_BYTE_PER_LINE,
      FDK_DUMP_BYTE_PER_LINE, FDK_DUMP_ASCII_LINE, FDK_DUMP_ASCII_COLUMN,
      WHITE_BLUE, "%s", asciiBuf);

  // Print Offset bar
  printWindowAt(pFdkUiProperty->fdkDumpPanel, offset, FDK_STRING_NLINE, 4,
      FDK_DUMP_OFF_LINE, FDK_DUMP_OFF_COLUMN, YELLOW_BLUE, "%4.4X",
  ...
  switch (pFdkUiProperty->fdkHwFunc) {
    ...
    case KHF_MEM:
      ...
      // Base address
      printWindowAt(pFdkUiProperty->fdkDumpPanel, baseaddr, FDK_STRING_NLINE,
          20, FDK_DUMP_BASEADDR_LINE,
          strlen(pFdkUiProperty->fdkDumpPanel.infoStr), WHITE_BLUE,
    ...
    case KHF_IO:
      ...
      // Base address
      printWindowAt(pFdkUiProperty->fdkDumpPanel, baseaddr, FDK_STRING_NLINE, 5,
          FDK_DUMP_BASEADDR_LINE, strlen(pFdkUiProperty->fdkDumpPanel.infoStr),
          WHITE_BLUE, FDK_INFO_IO_BASE_FMT,
    ...
    case KHF_PCI:
      ...
      if (pFdkPciDev)
        printWindowAt(pFdkUiProperty->fdkDumpPanel, baseaddr, FDK_STRING_NLINE,
            29, FDK_DUMP_BASEADDR_LINE,
            strlen(pFdkUiProperty->fdkDumpPanel.infoStr), WHITE_BLUE,
            FDK_INFO_PCI_BASE_FMT, pFdkPciDev->bus, pFdkPciDev->dev,
            pFdkPciDev->fun)
      ;

      // PCI first/second title
      printWindowMove(pFdkUiProperty->fdkDumpPanel, ftitle, FDK_STRING_NLINE,
          FDK_MAX_PCINAME, FDK_DUMP_FTITLE_LINE, FDK_DUMP_FTITLE_COLUMN,
          WHITE_BLUE, "%s: %s", FDK_FTITLE_PCI,
          (pFdkUiProperty->pFdkPciIds + pFdkUiProperty->fdkDumpPanel.byteBase)->venTxt)
      ;

      printWindowMove(pFdkUiProperty->fdkDumpPanel, stitle, FDK_STRING_NLINE,
          FDK_MAX_PCINAME, FDK_DUMP_STITLE_LINE, FDK_DUMP_FTITLE_COLUMN,
          WHITE_BLUE, "%s: %s", FDK_STITLE_PCI,
  ...
  // Highlight & Editing
  y = (pFdkUiProperty->fdkDumpPanel.byteOffset / FDK_DUMP_BYTE_PER_LINE)
      + FDK_DUMP_VALUE_LINE;
  x = ((pFdkUiProperty->fdkDumpPanel.byteOffset % FDK_DUMP_BYTE_PER_LINE) * 3)
      + FDK_DUMP_VALUE_COLUMN;
  if (pFdkUiProperty->fdkDumpPanel.toggleEditing) {

    color = (editorColorCount++ % 2) ? YELLOW_RED : YELLOW_BLACK;

    printWindowMove(pFdkUiProperty->fdkDumpPanel, highlight, FDK_STRING_NLINE,
        FDK_DUMP_HL_DIGITS, y, x, color, "%2.2X",
        pFdkUiProperty->fdkDumpPanel.editingBuf);
  } else {

    printWindowMove(pFdkUiProperty->fdkDumpPanel, highlight, FDK_STRING_NLINE,
        FDK_DUMP_HL_DIGITS, y, x, YELLOW_RED, "%2.2X",
        *(pDataPtr + pFdkUiProperty->fdkDumpPanel.byteOffset));

    pFdkUiProperty->fdkDumpPanel.editingBuf = *(pDataPtr
        + pFdkUiProperty->fdkDumpPanel.byteOffset);
  }
  ...
  if (pFdkUiProperty->fdkDumpPanel.toggleBits) {
    ...
    printWindowMove(pFdkUiProperty->fdkDumpPanel, bits, FDK_STRING_NLINE,
        FDK_DUMP_BITS_DIGITS, y + 1, x, WHITE_RED, "%d%d%d%d_%d%d%d%d",
  ...
  if (pFdkUiProperty->fdkHwFunc != KHF_PCI) {
    y = (pFdkUiProperty->fdkDumpPanel.byteOffset / FDK_DUMP_BYTE_PER_LINE)
        + FDK_DUMP_ASCII_LINE;
    x = (pFdkUiProperty->fdkDumpPanel.byteOffset % FDK_DUMP_BYTE_PER_LINE)
        + FDK_DUMP_ASCII_COLUMN;
    printWindowMove(pFdkUiProperty->fdkDumpPanel, hlascii, FDK_STRING_NLINE,
        FDK_DUMP_HLA_DIGITS, y, x, YELLOW_RED, "%c",
...
void handleKeyPressForDumpPanel(fdkUiProperty_t *pFdkUiProperty) {
  ...
  if (pFdkUiProperty->fdkDumpPanel.toggleEditing) {
    ...
    // Editing
    if ((pFdkUiProperty->inputBuf >= '0' && pFdkUiProperty->inputBuf <= '9')
        || (pFdkUiProperty->inputBuf >= 'a' && pFdkUiProperty->inputBuf <= 'f')
        || (pFdkUiProperty->inputBuf >= 'A' && pFdkUiProperty->inputBuf <= 'F')) {
  ...
  switch (pFdkUiProperty->inputBuf) {
    ...
    case KBPRS_UP:
      ...
      if ((pFdkUiProperty->fdkDumpPanel.byteOffset - FDK_DUMP_BYTE_PER_LINE)
          >= 0)
    ...
    case KBPRS_LEFT:
      ...
      if ((pFdkUiProperty->fdkDumpPanel.byteOffset - 1) >= 0
          && (pFdkUiProperty->fdkDumpPanel.byteOffset % FDK_DUMP_BYTE_PER_LINE)
              > 0)
        pFdkUiProperty->fdkDumpPanel.byteOffset--;
      else
    ...
    case KBPRS_RIGHT:
      ...
      if ((pFdkUiProperty->fdkDumpPanel.byteOffset + 1) < FDK_BYTE_PER_SCREEN
          && ((pFdkUiProperty->fdkDumpPanel.byteOffset + 1)
              % FDK_DUMP_BYTE_PER_LINE))
        pFdkUiProperty->fdkDumpPanel.byteOffset++;
      else
        pFdkUiProperty->fdkDumpPanel.byteOffset -= (FDK_DUMP_BYTE_PER_LINE - 1);
    ...
    case KBPRS_SPACE:
      ...
      pFdkUiProperty->fdkDumpPanel.toggleBits =
          !pFdkUiProperty->fdkDumpPanel.toggleBits;
      break;
