...
static void help( void ) {
    ...
    fprintf( stderr, "help: memvr ( [-r <address>] | [-w <address/value>] | [-d <address/length>] |\n" );
    fprintf( stderr, "                   [-o <address/value>] | [-a <address/value>] | -[hvbq] )\n\n");
    ...
    fprintf( stderr, "\t-d\tDump specified length of memory content from address\n\n" );
}
...
InvParm:
    ...
    // Check flags
    if( !(opflags & (MEMVR_MEM_READ | MEMVR_MEM_WRITE | MEMVR_MEM_DUMP | MEMVR_MEM_OR | MEMVR_MEM_AND)) ) {
    ...
    alignaddr = address & 0xFFFFFFFFFFFFE000;
    offset = (address - alignaddr) / 4;
