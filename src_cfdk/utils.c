...
s32 connectToFdkServer(fdkUiProperty_t *pFdkUiProperty) {
  ...
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_CONNECT, 0, 0,
  NULL, pFdkUiProperty->pktBuf,
  FDK_MAXSZ_PKT);
}
...
void disconnectFromFdkServer(fdkUiProperty_t *pFdkUiProperty) {
  ...
  executeFunction(pFdkUiProperty->fd, FDK_REQ_DISCONNECT, 0, 0,
  NULL, pFdkUiProperty->pktBuf,
  FDK_MAXSZ_PKT);
}
...
s32 readPciList(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read PCI list
  if (executeFunction(pFdkUiProperty->fd, FDK_REQ_PCI_LIST, 0, 0, NULL,
      pFdkUiProperty->pktBuf, FDK_MAXSZ_PKT))
    return 1;
  ...
  memcpy(pFdkUiProperty->pFdkPciDev, &pFdkRspPciListPkt->pciListContent,
      sizeof(fdkPciDev_t) * pFdkUiProperty->numOfPciDevice);
  ...
  for (i = 0; i < pFdkUiProperty->numOfPciDevice; i++) {
    ...
    getPciVenDevTexts((pFdkUiProperty->pFdkPciDev + i)->vendorId,
        (pFdkUiProperty->pFdkPciDev + i)->deviceId,
        (pFdkUiProperty->pFdkPciIds + i)->venTxt,
        (pFdkUiProperty->pFdkPciIds + i)->devTxt, pFdkUiProperty->pciIdsPath);
  }
...
s32 readE820List(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read E820 list
  if (executeFunction(pFdkUiProperty->fd, FDK_REQ_E820_LIST, 0, 0, NULL,
      pFdkUiProperty->pktBuf, FDK_MAXSZ_PKT))
    return 1;
  ...
  pFdkUiProperty->pFdkE820record = (fdkE820record_t *) malloc(
      sizeof(fdkE820record_t) * pFdkUiProperty->numOfE820Record);

  if (!pFdkUiProperty->pFdkE820record)
    return 1;

  memcpy(pFdkUiProperty->pFdkE820record, &pFdkRspE820ListPkt->e820ListContent,
      sizeof(fdkE820record_t) * pFdkUiProperty->numOfE820Record);
...
s32 readMemory(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read memory
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_MEM_READ,
...
s32 writeMemoryByEditing(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Write memory
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_MEM_WRITE,
      pFdkUiProperty->fdkDumpPanel.byteBase
          + pFdkUiProperty->fdkDumpPanel.byteOffset,
      sizeof(pFdkUiProperty->fdkDumpPanel.editingBuf),
      &pFdkUiProperty->fdkDumpPanel.editingBuf, pFdkUiProperty->pktBuf,
      FDK_MAXSZ_PKT);
}
...
s32 readIo(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read io
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_IO_READ,
      pFdkUiProperty->fdkDumpPanel.byteBase,
      FDK_BYTE_PER_SCREEN,
      NULL, pFdkUiProperty->pktBuf,
      FDK_MAXSZ_PKT);
...
s32 writeIoByEditing(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Write io
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_IO_WRITE,
      pFdkUiProperty->fdkDumpPanel.byteBase
          + pFdkUiProperty->fdkDumpPanel.byteOffset,
      sizeof(pFdkUiProperty->fdkDumpPanel.editingBuf),
      &pFdkUiProperty->fdkDumpPanel.editingBuf, pFdkUiProperty->pktBuf,
...
s32 readIde(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read ide
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_IDE_READ,
      pFdkUiProperty->fdkDumpPanel.byteBase,
      FDK_BYTE_PER_SCREEN,
      NULL, pFdkUiProperty->pktBuf,
      FDK_MAXSZ_PKT);
...
s32 writeIdeByEditing(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Write ide
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_IDE_WRITE,
      pFdkUiProperty->fdkDumpPanel.byteBase
          + pFdkUiProperty->fdkDumpPanel.byteOffset,
      sizeof(pFdkUiProperty->fdkDumpPanel.editingBuf),
      &pFdkUiProperty->fdkDumpPanel.editingBuf, pFdkUiProperty->pktBuf,
...
s32 readCmos(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read cmos
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_CMOS_READ,
      pFdkUiProperty->fdkDumpPanel.byteBase,
      FDK_BYTE_PER_SCREEN,
      NULL, pFdkUiProperty->pktBuf,
      FDK_MAXSZ_PKT);
...
s32 writeCmosByEditing(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Write cmos
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_CMOS_WRITE,
      pFdkUiProperty->fdkDumpPanel.byteBase
          + pFdkUiProperty->fdkDumpPanel.byteOffset,
      sizeof(pFdkUiProperty->fdkDumpPanel.editingBuf),
      &pFdkUiProperty->fdkDumpPanel.editingBuf, pFdkUiProperty->pktBuf,
...
s32 readPci(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Read PCI
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_PCI_READ,
      calculatePciAddress(pFdkPciDev->bus, pFdkPciDev->dev, pFdkPciDev->fun),
      FDK_BYTE_PER_SCREEN,
      NULL, pFdkUiProperty->pktBuf,
      FDK_MAXSZ_PKT);
}
...
s32 writePciByEditing(fdkUiProperty_t *pFdkUiProperty) {
  ...
  // Write PCI
  return executeFunction(pFdkUiProperty->fd, FDK_REQ_PCI_WRITE,
      calculatePciAddress(pFdkPciDev->bus, pFdkPciDev->dev, pFdkPciDev->fun)
          + pFdkUiProperty->fdkDumpPanel.byteOffset,
      sizeof(pFdkUiProperty->fdkDumpPanel.editingBuf),
      &pFdkUiProperty->fdkDumpPanel.editingBuf, pFdkUiProperty->pktBuf,
